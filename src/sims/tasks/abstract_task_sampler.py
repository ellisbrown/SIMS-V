...
from sims.utils.constants.stretch_initialization_utils import PHYSICS_SETTLING_TIME
from sims.utils.data_generation_utils.exception_utils import (
    HouseInvalidForTaskException,
    TaskSamplerInInvalidStateError,
)
from sims.utils.type_utils import Vector3, AbstractTaskArgs, KeyedDefaultDict

if TYPE_CHECKING:
    from sims.environment.stretch_controller import StretchController


class AbstractSimsTaskSampler(TaskSampler):
    def __init__(
        self,
        ...
        **kwargs: Any,
        ...
        assert PHYSICS_SETTLING_TIME == settle_physics_for_second_when_reset, (
            "Physics settling time is fixed for deterministic generation."
        )
        ...
        # Use CloudRendering on Linux with GPU.
        # Note: Linux64 builds are never generated (AUTO_BUILD_PLATFORMS = [CloudRendering, OSXIntel64]).
        # CloudRendering requires libvulkan.so.1 — provide via LD_LIBRARY_PATH in SLURM scripts.
        ...
        self._last_sampled_task: Optional[AbstractSimsTask] = None
            ...
            if retain_agent_pose:
                ...
                if self.controller.get_current_agent_full_pose()["position"]["y"] < 0:
                    ...
                    for room_id in sorted(self.controller.room_poly_map.keys()):
                        candidate_points = self.controller.get_candidate_points_in_room(
                            room_id
                        )
                        for cand in candidate_points:
                            event = self.controller.teleport_agent(
                                position={
                                    "x": float(cand[0]),
                                    "y": self.current_house["metadata"]["agent"][
                                        "position"
                                    ]["y"],
                                    "z": float(cand[1]),
                                },
                                rotation=self.current_house["metadata"]["agent"][
                                    "rotation"
                                ],
                            )
                            if event:
