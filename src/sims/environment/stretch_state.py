...
from sims.utils.constants.stretch_initialization_utils import AGENTS_BASE_HEIGHT
...
class StretchState:
    # Fixed state limits used even when no controller instance is available.
    arm_extreme_values = {
        "lift_max": 1.0457,
        "lift_min": -0.055,
    ...
    agent_center_y_height = AGENTS_BASE_HEIGHT

    arm_coord_offsets = {
        "translation": {"x": -0.1157, "y": -0.739992, "z": -0.0239},
        "rotation": {"theta": 90.0},
    }  # Transform between the agent-pivot and arm coordinate frames.

    # Approximate real-robot limit; visibility checks still apply.
    max_interactable_height = 1.2
    ...
    def __init__(self, controller=None):
        if controller is not None:
            ...
            held_oids = set(controller.last_event.metadata["arm"]["heldObjects"] or [])
    ...
    def _delta_held_oids(cls, after_state, before_state):
        ...
        return {(False, oid) for oid in deletions} | {(True, oid) for oid in additions}
    ...
    def create_delta_from_goal(cls, current_state: "StretchState", goal_dict):
        ...
        return cls.difference(
            final_state=goal_state, initial_state=current_state
        ), goal_state
    ...
    def _create_difference_state(
        ...
        difference_state = StretchState(None)
    ...
    def state_change_within_tolerance(
        ...
        less than 1 cm away from the target, then the arm cannot get closer in the discrete Stretch action space).
    ...
    def get_wrist_pose(controller):
        # These offsets make AI2-THOR joint metadata comparable with the real
        # Stretch robot's agent-relative wrist coordinate frame.
...
def calc_inverse_full_rotation(full_rotation):
    rotation = R.from_euler(
        "xyz",
        [full_rotation["x"], full_rotation["y"], full_rotation["z"]],
        degrees=True,
    )
...
def convert_world_to_relative_coordinate(
    ...
    if "theta" in relative_rotation:
        # A theta-only pose uses the yaw-specific inverse rotation.
        inverse_agent_rotation = calc_inverse_only_y(relative_rotation["theta"])
    elif (
        "x" in relative_rotation
        and "y" in relative_rotation
        and "z" in relative_rotation
    ):
        inverse_agent_rotation = calc_inverse_full_rotation(relative_rotation)
    else:
        raise ValueError(
            "relative_rotation must have either 'theta' or 'x', 'y', 'z' keys"
        )
...
def convert_world_to_relative_coordinate_batched(world_obj, relative_location_rotation):
    agent_translation = np.array(
        [
            relative_location_rotation["x"],
            relative_location_rotation["y"] or 0,
            relative_location_rotation["z"],
        ]
    )
    if "theta" in relative_location_rotation:
        inverse_agent_rotation = calc_inverse_only_y(
            relative_location_rotation["theta"]
        )
    else:
        assert (
            "rotation" in relative_location_rotation
            and "x" in relative_location_rotation["rotation"]
            and "y" in relative_location_rotation["rotation"]
            and "z" in relative_location_rotation["rotation"]
        )
        inverse_agent_rotation = calc_inverse_full_rotation(
            relative_location_rotation["rotation"]
        )

    # Note that we will be ignoring thetas for final location
    if (
        isinstance(world_obj, list)
        and len(world_obj) > 0
        and isinstance(world_obj[0], dict)
    ):
        world_positions = np.array(
            [[obj["x"], obj["y"], obj["z"]] for obj in world_obj]
        )
    else:
        assert isinstance(world_obj, np.ndarray) and world_obj.shape[1] == 3
        world_positions = world_obj
    obj_translation = np.matmul(
        inverse_agent_rotation, (world_positions - agent_translation).transpose()
    )
    # add rotation later
    result = obj_translation.transpose()
    return result
